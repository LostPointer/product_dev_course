...
**Как применить класс к сетям в KiCad 9.0:**

1. **Создайте класс трассировки:**
   - Откройте `File → Board Setup` (или нажмите `Alt+P`)
   - Перейдите на вкладку **"Net Classes"**
   - Нажмите кнопку **"+"** (Add Net Class) или **"New Class"**
   - Введите имя класса: `Power` или `3V3_Power`
...
2. **Назначьте сети классу:**
   - В том же окне **"Net Classes"** выберите созданный класс `Power` (или `3V3_Power`)
   - В правой части окна найдите список **"Net Assignments"** или **"Assigned Nets"**
   - Нажмите кнопку **"+"** рядом со списком сетей
   - В появившемся списке выберите сети:
     - `+3.3V`
     - `3V3`
     - `LSM_VDD`
     - Любые другие сети питания 3.3V
   - Или используйте поиск: введите `3.3` или `3V3` в поле поиска
   - Нажмите **"OK"** для каждой выбранной сети

3. **Альтернативный способ (через список сетей):**
   - В окне **"Net Classes"** внизу есть список всех сетей платы
   - Найдите нужные сети (`+3.3V`, `3V3`, `LSM_VDD`)
   - В колонке **"Net Class"** для каждой сети выберите из выпадающего списка класс `Power`
   - Или выделите несколько сетей (Ctrl+Click) и измените класс для всех сразу

4. **Проверка:**
   - После назначения класса, при трассировке этих сетей будут автоматически использоваться параметры класса:
     - Ширина дорожки: 0.5 mm
     - Размер переходных отверстий: 0.6 mm
     - Зазор: 0.2 mm

**Примечание:** Если сеть не отображается в списке, убедитесь, что она используется в схеме (schematic) и обновлена связь между схемой и платой через `Tools → Update PCB from Schematic` (или `F8`).

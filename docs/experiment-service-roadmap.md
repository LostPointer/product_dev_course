...
- **Множественные проекты при создании датчика:** ❌ Не реализовано. При создании датчика должна быть возможность сразу добавить его в несколько проектов, а не только в один. Требуется доработка UI в `CreateSensor.tsx` для выбора нескольких проектов (multi-select) и обновление API endpoint `POST /api/v1/sensors` для поддержки массива `project_ids` или отдельного вызова `POST /api/v1/sensors/{id}/projects` после создания.
...
- **Автозаполнение проекта при создании эксперимента:** ❌ Не реализовано. При открытии списка экспериментов с выбранным проектом, при создании нового эксперимента поле "Проект" должно быть автоматически заполнено выбранным проектом. Требуется доработка UI в `CreateExperiment.tsx` для передачи `project_id` из контекста списка экспериментов.

...
- **Контроль доменных инвариантов:** ⚠️ Частично реализовано. Валидация переходов статусов реализована в `experiment-service/services/state_machine.py`. **Добавлен инвариант:** нельзя перевести `Run` в `succeeded/failed/archived`, если есть активные `CaptureSession` (draft/running/backfilling); применяется в `PATCH /api/v1/runs/{id}` и `POST /api/v1/runs:batch-status`. Остальные инварианты (например, удаление датчика при активных сессиях) — в backlog.

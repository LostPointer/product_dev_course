...
После создания датчика токен отображается около 2 секунд — не успеваешь прочитать и скопировать.

**Ожидаемое поведение:** токен остаётся на экране до явного закрытия, или есть кнопка «Скопировать» + предупреждение, или длительный/отключаемый таймер.
...
После тестовой отправки сообщения датчику происходит выход из системы и редирект на страницу входа.

**Возможные причины:** ответ API с 401/403 и глобальный interceptor очищает сессию; неверный эндпоинт тестовой отправки; истечение токена в момент запроса; обработка ошибки как «неавторизован».

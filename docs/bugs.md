...
При отпускании левой кнопки мыши вне области модального окна «Создать проект» окно закрывается (ожидаемое поведение для многих диалогов — закрытие по клику по backdrop, но воспринимается как баг: срабатывает на mouseup вне области, а не только на явном закрытии).

**Возможные причины:** обработка `onMouseUp` / `onClickOutside` на backdrop без различия mousedown внутри vs снаружи; закрытие по полному click-циклу, начатому вне модалки.
...
**Возможные причины:** маршрут фронта не зарегистрирован / переименован; запрос к бэкенду уходит на несуществующий эндпоинт; неверная маршрутизация в auth-proxy (аналогично BUG-B-003 — неправильный апстрим для логов).
...
При попытке создать системную роль выдаётся ошибка:
```
Ошибка
×
...
**Возможные причины:**
- Запрос не содержит CSRF-токен или токен невалидный/истёкший
- CSRF-токен не передаётся в заголовке или body
- auth-proxy не пробрасывает CSRF-токен до бэкенда
- Бэкенд требует CSRF-защиту для POST /api/v1/roles, но она не конфигурируется в auth-proxy
...
Неактивный инвайт, по которому уже зарегистрировались, в столбце «Использован» показывает прочерк (`-`) вместо даты использования.

**Возможные причины:**
- Дата использования инвайта не сохраняется в БД при регистрации
- Фронт не получает дату использования в ответе от бэкенда
- Фронт получает дату, но отображает её как прочерк (например, если значение `null` или пустая строка)
...
При попытке открыть Аудит-лог выдаётся ошибка:
```
Ошибка
×
...
**Возможные причины:** эндпоинт аудит-лога не реализован в бэкенде; неверная маршрутизация в auth-proxy (аналогично BUG-B-003 / BUG-F-009); фронт отправляет запрос на несуществующий URL.
